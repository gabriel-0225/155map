...
3. Netlify  
   当前项目已部署到 Netlify 项目 `155map`。当需要发布新版时，提交并推送到 GitHub，再部署到同一个 Netlify 站点，即可让访问者看到最新版网页。
...
普通修改默认执行本地检查，并提交到 Git 作为本地保存点。

普通修改不自动推送 GitHub、不自动部署 Netlify。

当明确要求提交 GitHub 或发布线上版本时，再执行：
...
以后由 Codex 修改本项目时，默认会提交本地 Git；只有在用户明确要求提交 GitHub 或发布时，才同步线上版本。
